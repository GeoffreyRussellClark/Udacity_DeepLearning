...
with graph.as_default():
  ...
  #sparse tensor of indicies to do sparse embeddings lookup

  """
  ########## Not sure how to generate an index for the sparse tensor :/ ##################
  avg_embed = None
  #sparse_indices = tf.constant([batch_size,1], dtype=tf.int64)
  for i in range(batch_size):
      sp_train_dataset = tf.SparseTensor(indices=[np.arange(batch_size,1)], values=train_dataset[i,:],dense_shape=train_dataset.shape[1])

      if avg_embed is None:
          avg_embed = tf.nn.embedding_lookup_sparse(embeddings, sp_train_dataset, None, combiner="mean")
      else:
          avg_embed = tf.concat([avg_embed,tf.nn.embedding_lookup_sparse(embeddings, sp_train_dataset, None, combiner="mean")],1)
